...
func TestInitCreatesClaudeSettingsJsonInProject(t *testing.T) {
	dir := t.TempDir()
...
func TestInitOutputConfirmsHookIsActive(t *testing.T) {
	dir := t.TempDir()
	cmd := exec.Command(ensembleBinAbs(t), "init")
	cmd.Dir = dir
	out, err := cmd.CombinedOutput()
...
func TestInitIsIdempotentWhenRunTwice(t *testing.T) {
	dir := t.TempDir()
...
func TestInitMergesHookWhenSettingsJsonAlreadyHasOtherContent(t *testing.T) {
	dir := t.TempDir()
...
func TestInitWarnsWhenEnsembleIsNotOnPath(t *testing.T) {
	dir := t.TempDir()
	cmd := exec.Command(ensembleBinAbs(t), "init")
	cmd.Dir = dir
